...
.requires(
	...
	'weltmeister.undo',
	'plugins.wm.level-properties' //lprop plugin
)
...
wm.Weltmeister = ig.Class.extend({	
	...
	layers: [],
	entities: null,
	levelproperties: null,  //lprop plugin
	activeLayer: null,
	collisionLayer: null,
	selectedEntity: null,
	...
	init: function() {
		...
		this.levelproperties = new wm.LevelProperties();  //lprop plugin
	...
	loadNew: function() {
		...
		this.screen = {x: 0, y: 0};
		this.entities.clear();
		this.levelproperties.clear(); //lprop plugin
	...
	loadResponse: function( data ) {
		...
		for (var key in data.properties) {
		    this.levelproperties.add(key, data.properties[key]);
		}
		//lprop plugin end
	...
	save: function( dialog, path ) {
		...
		this.fileName = path.replace(/^.*\//,'');
		var data = {
			'properties': this.levelproperties.getSaveData(),  //lprop plugin
